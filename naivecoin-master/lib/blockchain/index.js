...
class Blockchain {
    constructor(dbName) {
        ...
        this.certificatesDb = new Db('data/' + dbName + '/' + CERTIFICATES_FILE, new Transactions());
        ...
        this.certificates = this.certificatesDb.read(Transactions);
    ...
    getCertificateById(id) {
        if (!id) {
            console.error('getCertificateById called with undefined id');
            return null;
        }
        return R.find(R.propEq('id', id), this.certificates);
    }

    getCertificateFromBlocks(certificateId) {
   ...
   replaceChain(newBlockchain) {
    ...
    addTransaction(newTransaction, emit = true) {
        ...
        if (this.checkTransaction(newTransaction, this.blocks)) {
            ...
            this.certificates.push(newTransaction);
            this.certificatesDb.write(this.transactions);
    ...
    addCertificate(newCertificate, emit = true) {
        // It only adds the certificate if it's valid

        if (!newCertificate || !newCertificate.id || !newCertificate.hash) {
            throw new Error('Invalid certificate: Missing required fields');
        }

        if (this.checkCertificate(newCertificate, this.blocks)) {

            this.certificates.push(newCertificate);
            this.certificatesDb.write(this.certificates);

            console.info(`Certificate added: ${newCertificate.id}`);
            console.debug(`Certificate added: ${JSON.stringify(newCertificate)}`);
            if (emit) this.emitter.emit('CertificateAdded', newCertificate);
    ...
    checkCertificate(certificate, referenceBlockchain = this.blocks) {

        console.log('Reference Blockchain:', referenceBlockchain);
        // Verify if the certificate isn't already in the blockchain
        let isNotInBlockchain = R.all((block) => {
            // console.log('Block:', block);
            // console.log('Certificates:', block.certificates);
            return R.none(R.propEq('id', certificate.id), block.certificates);
        }, referenceBlockchain);

        console.log('Certificate ID:', certificate.id);

        if (!isNotInBlockchain) {
            console.error(`Certificate '${certificate.id}' is already in the blockchain`);
            throw new TransactionAssertionError(`Certificate '${certificate.id}' is already in the blockchain`, certificate);
        }
        return true;
    }
